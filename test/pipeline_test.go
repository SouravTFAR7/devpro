...
func TestTerraformAwsCodePipeline(t *testing.T) {
	...
	fmt.Println("Waiting for CodePipeline execution to succeed...")
	waitForPipelineSuccess(t, cpClient, pipelineName)

	// --- Optional: Test EC2 Deployment (only if output is present) ---
	ec2PublicIp := ""
	if output, err := terraform.OutputE(t, terraformOptions, "ec2_public_ip"); err == nil && output != "" {
		ec2PublicIp = output
		fmt.Println("Testing deployed app on EC2 at:", ec2PublicIp)
		...
		fmt.Println("No EC2 output found — skipping live app test.")
	}
}
...
func waitForPipelineSuccess(t *testing.T, cpClient *codepipeline.CodePipeline, pipelineName string) {
	...
	for i := 0; i < retries; i++ {
		...
		if allSucceeded {
			fmt.Println("✅ CodePipeline execution succeeded.")
			return
		}
